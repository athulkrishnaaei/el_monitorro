...
fn parse_int_from_string(command: &str) -> Option<std::string::String> {
    ...
    match data {
        Some(s) => Some(s),
        None => Some("error fetching message".to_string()),
    }
}
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        ...
        let result = match st {
            ...
            st if st.starts_with(ListSubscriptions::callback()) => {
                ...
                let feed_url = get_feed_url_by_id(db_pool, feedid.clone().unwrap());
            ...
            st if st.starts_with(Unsubscribe::callback()) => {
                let feedid = parse_int_from_string(&st);
                let feed_url = get_feed_url_by_id(db_pool, feedid.clone().unwrap());
                CallbackDatas::CallbackUnsubscribe(feedid, feed_url)
            }
    ...
    fn process_callback_query(
        ...
        match command {
            CallbackDatas::ListSubscriptions => ListSubscriptions::builder()
                ...
                .build()
                .run(db_pool, api, message),
            CallbackDatas::CallbackListSubscriptions(feedid, feed_url) => match feedid {
                Some(feedid) => {
                    ...
                    let send_message_params =
                        ListSubscriptionsInlineKeyboard::set_list_subcriptions_menu_keyboard(
                            message, feedid, feed_url,
                        );
                    api.send_message(&send_message_params).unwrap();
                }
                None => ListSubscriptions::builder()
                    .message(message.clone())
                    .build()
                    .run(db_pool, api, message),
            },
            ...
            CallbackDatas::CallbackSetTemplate(feedid) => match feedid {
                ...
                None => {
                    let args = parse_args(SetTemplate::command(), commands);
                    SetTemplate::builder()
                        .message(message.clone())
                        .args(args)
                        .build()
            ...
            CallbackDatas::CallbackUnsubscribe(feedid, feed_url) => match feedid {
                Some(_feedid) => {
                ...
                None => {
                    let args = parse_args(Unsubscribe::command(), commands);
