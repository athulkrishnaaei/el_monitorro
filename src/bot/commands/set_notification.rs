...
static COMMAND: &str = "/set_notification";
static CALLBACK: &str = "set_notification";

pub struct SetNotification {}

impl SetNotification {
    ...
    fn set_notification(
        ...
        match telegram::set_subscriptions_has_notification(db_connection, &subscription, argument) {
            Ok(_) => {
                if vec[1] == "disable" {
                    "Notifications Disabled for your messages:".to_string()
                } else {
                    "Notifications Enabled for your messages".to_string()
                }
            }
...
impl Command for SetNotification {
    fn response(
        &self,
        ...
        match self.fetch_db_connection(db_pool) {
            Ok(mut connection) => {
                ...
                self.set_notification(&mut connection, message, argument)
            }
